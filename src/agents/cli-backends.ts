...
const DEFAULT_TRAE_BACKEND: CliBackendConfig = {
  command: "trae-cli",
  args: ["run"],
  output: "text",
  input: "arg",
...
export function resolveCliBackendConfig(
  ...
  if (normalized === "trae-cli") {
    const merged = mergeBackendConfig(DEFAULT_TRAE_BACKEND, override);
    const command = merged.command?.trim();
    if (!command) {
      return null;
    }
