...
describe("runCliAgent with process supervisor", () => {
  ...
  it("passes nested provider/model refs through providerArg and modelArg", async () => {
    supervisorSpawnMock.mockResolvedValueOnce(
      createManagedRun({
        reason: "exit",
        exitCode: 0,
    ...
    expect(input.argv).toContain("trae-cli");
    expect(input.argv).toContain("--provider");
    expect(input.argv).toContain("openai");
    expect(input.argv).toContain("--model");
    expect(input.argv).toContain("gpt-4o");
  });
