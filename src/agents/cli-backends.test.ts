...
describe("resolveCliBackendConfig trae-cli defaults", () => {
  it("registers trae-cli as a built-in CLI backend", () => {
    const ids = resolveCliBackendIds();

    expect(ids.has("trae-cli")).toBe(true);
  });

  it("uses text output and disables session resume by default", () => {
    const resolved = resolveCliBackendConfig("trae-cli");

    expect(resolved).not.toBeNull();
    expect(resolved?.config.command).toBe("trae-cli");
    expect(resolved?.config.args).toEqual(["run"]);
    expect(resolved?.config.output).toBe("text");
    expect(resolved?.config.sessionMode).toBe("none");
    expect(resolved?.config.providerArg).toBe("--provider");
    expect(resolved?.config.modelArg).toBe("--model");
  });

  it("retains watchdog defaults when command is overridden", () => {
    const cfg = {
      agents: {
        defaults: {
          cliBackends: {
    ...
    const resolved = resolveCliBackendConfig("trae-cli", cfg);

    expect(resolved).not.toBeNull();
    expect(resolved?.config.command).toBe("/usr/local/bin/trae-cli");
    expect(resolved?.config.output).toBe("text");
    expect(resolved?.config.sessionMode).toBe("none");
    expect(resolved?.config.reliability?.watchdog?.fresh?.noOutputTimeoutRatio).toBe(0.8);
    expect(resolved?.config.reliability?.watchdog?.resume?.noOutputTimeoutRatio).toBe(0.3);
