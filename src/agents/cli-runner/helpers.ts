...
export function resolveCliProviderAndModel(params: {
  ...
  const normalizedModel = normalizeCliModel(params.modelId, params.backend);
  if (!params.backend.providerArg) {
  ...
  return {
    providerId: normalizedModel.slice(0, slash),
    modelId: normalizedModel.slice(slash + 1),
  };
}
...
export function buildCliArgs(params: {
  ...
  if (!params.useResume && params.backend.providerArg && params.providerId) {
    args.push(params.backend.providerArg, params.providerId);
  }
